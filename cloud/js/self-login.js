~ function(window) {
    ...
    window.viewYhtUser = function(destUrl, htmlUrl, option) {
        ...
        var iframeHtml = '<div id="yhtAppendCover" style="display:none;"><div id="yhtAppendIframe" style="background-color: transparent;opacity: 1;text-align:center;position: fixed;' +
            ...
            '<div  id="yhtFrameLogin" style="display:none;">' +
        ...
        $('#toyhtFrameReg').click(function() {
            $('#yhtregIframe').show();
            $('#toyhtFrameReg').css('color', '#e50113').css('borderBottomColor', '#e50113');
            $('#yhtFrameLogin').hide();
            $('#toyhtFrameLogin').css('color', '#636363').css('borderBottomColor', 'transparent');

        });

        $('#toyhtFrameLogin').click(function() {
            var _this = $('#yhtloginIframe');
            if (!_this.attr('src')) {
                _this.attr('src', _this.attr('__src'));
            }
            $('#yhtFrameLogin').show();
            // document.getElementById("toyhtFrameLogin").style.color = '#e50113';
            // document.getElementById("toyhtFrameLogin").style.borderBottomColor = '#e50113';
            $('#toyhtFrameLogin').css('color', '#e50113').css('borderBottomColor', '#e50113');
            $('#yhtregIframe').hide();
            // document.getElementById("toyhtFrameReg").style.color = '#636363';
            // document.getElementById("toyhtFrameReg").style.borderBottomColor = 'transparent';
            $('#toyhtFrameReg').css('color', '#636363').css('borderBottomColor', 'transparent');
        });
    ...
    window.yhtjsonpcallback = function(respon) {
        ...
        if (respon.status == 1 && 　!respon.needrelogin) {
            ...
            $('#yhtloginIframe').attr('__src', loginUrl);
    ...
    window.yhtOpenTarget = function(realservice) {
        ...
        // $('body').append("<form id='yhthiddenlink' action='"+realservice+"' target='_blank'><input type='hidden' name='object' value='test'></form>");
        // $('#yhthiddenlink').submit();
    }
}(window);